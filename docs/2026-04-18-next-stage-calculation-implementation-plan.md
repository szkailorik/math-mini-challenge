...
把第二阶段的“空壳 + 第一版纸面结构”做出来，让它能以“总收束 + 旧知保温”的方式独立生成四张题卷并稳定打印。
...
M2 只要求把第二阶段基础纸面跑起来，不要求一次做完整题库，但题卷必须明确体现“总收束 + 旧知保温”。
...
> 实施状态更新：`v23.23` 已完成这一里程碑的第一版落地，包括 `readiness 状态词`、`promotion gate`、`继续巩固 7 套` 分支、`transfer bootstrap`、`program-aware set counter` 和对应 runtime validation。
