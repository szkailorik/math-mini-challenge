...
- v23.255 startup guardrail: `startEngine` now calls `renderStartupLocalPaper()` before waiting on cloud sync, then runs `StorageDB.init({ skipFallback: true })` with a short startup timeout so the worksheet appears from local cache first; when cloud sync later finishes, `refreshAfterStartupCloudSync` refreshes only if the user is still on the main paper view, and the optional `html2canvas` export dependency is injected by `ensureHtml2CanvasLoaded()` only when image export is clicked, so a slow CDN cannot block the first paint.
