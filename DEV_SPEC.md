...
- `showCalculationQuickReview`: renders the child-facing `16大终极心智模型 / 名师词典` page using a stable 16-model registry, an all/core/advanced screen filter, a four-route model-selection layer, core/advanced model labels, per-model `什么时候用` cues, per-model `先判断` recognition checkpoints, per-model `别混淆` discrimination cues, stage-aware emphasis copy, recommendation chips, jump-to-topic helpers, and dedicated print markup.
