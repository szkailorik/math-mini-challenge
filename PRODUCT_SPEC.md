...
- Error-book grading and result pages return to the same learner's error book.
- This reduces cross-learner confusion when a tutor moves between KAI and Lorik during one review session.
...
- The parent can print first, check answers later, and enter paper grades without losing one-to-one alignment.
- The learning purpose is short spaced retrieval: recommended 10-question packs stay the daily default, while 20-question and full-scope packs remain heavier cleanup options.
...
- Batch refill includes all three paper states: mastered, needs explanation, and wrong-again.
- Result reports use parent-facing wording for the middle state instead of implying ordinary careless slips.
...
- Parents can mark all rows as mastered or wrong-again, then adjust exceptions before submitting.
- This keeps feedback capture immediate enough to preserve error-book scheduling while respecting paper-first workflows.
...
- The cue targets the likely wrong step rather than repeating the full explanation.
- Print output keeps the cue beside the answer so parent-led review can diagnose quickly.

v23.201 adds one aligned micro drill to every model-dictionary entry:

- Each drill is hand-written for the target model instead of sampled from the general question bank.
- Screen answers are collapsed so learners retrieve first and check second.
- Print output exposes parent-facing answers for paper-based oral review.
...
- This lowers cognitive load for daily review while keeping advanced models available.
- Print view still includes the full 16-model dictionary so paper review does not miss content.
...
- The checkpoint trains retrieval and model recognition before worked-solution reading.
- These checkpoints appear in screen and print views for parent-led oral review.
...
- Model 16 keeps `方程逆运算模型`, so inverse-equation reasoning remains visible instead of being hidden inside quantity relations.
- The model teaches the canonical flow: assume all one type, compute total difference, divide by per-item difference, and substitute back to check.
- The dictionary now has 13 core models plus 3 advanced models: fraction division, chicken-rabbit assumption, and equation inverse reasoning.

v23.191 rebuilds the teacher dictionary as a lower-noise model dictionary:

- The quick-review page is now positioned as `16大终极心智模型 / 名师词典`.
...
- Passing variants show `贴合原题`; risky variants show `需复核` with the warning reason.
- Badges appear in screen and print contexts so tutors can make item-level assignment decisions without relying only on the audit summary.
...
- The quality gate rejects candidates whose skeleton no longer matches the source mistake when a better candidate exists.
- This raises the precision of retrieval practice by keeping variants close to the original error mechanism while still changing surface numbers.
...
- Backup second-pass printing now supports question-only, answer-only, and combined-but-separated modes.
- Runtime validation asserts the answer-only shell does not include the learner-facing backup question bank.
...
- Result tables show a `对应原错题` column when source metadata exists, so tutors can match a backup item back to the original worksheet question.
- Reopened recent practice logs retain this source label alongside the backup question, result, and correct answer.
...
- Backup grading uses the same `已会 / 粗心 / 又错` feedback model as targeted error-book practice, preserving source error UIDs so results update the original mistake record.
- Follow-up printing and backup grading resolve the report's own Set number, so reopening an older report does not accidentally print or grade the current set.
...
- v23.228 persists generated variant packs onto the set history after first preview/print/grading, so refreshes, later printing, and cloud sync can reuse the same reviewed questions until the tutor explicitly regenerates them.
- v23.229 adds a preview audit summary before printing: the main and backup preview pages now show whether the pack is ready to print, how many source mistakes/main variants/backup variants were matched, how large the candidate pool was, and whether any duplicate, drift, or fit risks need review.
- v23.230 improves variant quality for fraction mistakes: fallback same-set variants now read the original fraction structure before generating, preserving add/subtract/multiply/divide, parenthesized expressions, mixed numbers, and operation order instead of falling back to a generic fraction item.
- v23.231 improves variant quality for relation mistakes: fallback equation variants now preserve unknown-number position, and fallback unit-rate variants preserve whether the source asked for speed, distance, total price, unit price, or unit conversion.
- v23.232 improves variant quality for decimal division mistakes: fallback variants now preserve which side has decimals and whether the quotient is less than 1, so place-value recovery does not degrade into generic division or mixed arithmetic.
- v23.233 improves variant quality for conversion mistakes: fraction-to-decimal, fraction-to-percent, and decimal-to-percent fallback variants now preserve conversion direction, while HTML fraction markup no longer gets mistaken for a comparison sign.
- v23.234 improves variant quality for complex mixed mistakes: near-100 compensation, operation order, parentheses, friendly-number shortcuts, distributive-law, telescoping fractions, and fraction-of-total remainder mistakes now use source-aware structures instead of generic mixed-practice lookalikes.
- v23.235 improves variant quality for estimation and validation mistakes: comparison-to-one, estimate-the-nearest-result, and reasonableness-check prompts now keep the same judgement task instead of drifting into ordinary calculation.
- v23.236 improves variant quality for unit-conversion mistakes: length and mass conversions now preserve direction and target unit, including cm-to-m, kg-and-g-to-g, and mixed-unit operations.
- v23.237 improves variant quality for integer arithmetic mistakes: continuous-zero borrowing, jump-zero borrowing, scale-by-10 division, middle-zero quotients, near-100 multiplication, and friendly-25 multiplication now keep their specific error mechanism instead of only preserving the operation sign.
- v23.238 improves variant quality for equation mistakes: known inverse-equation tags and Closure percent-equation tags now prefer same-structure source-aware variants, preserving unknown position and direct-vs-offset percent-equation form before broader boundary transfer.
- v23.239 improves variant quality for Closure bridge mistakes: ratio-to-decimal, fraction-to-ratio, representation-choice, baseline-comparison, and post-conversion-use mistakes now keep the same bridge action instead of drifting into generic conversion lookalikes.
- v23.240 simplifies answer-key checking: main, Closure, follow-up, and backup answer references now prioritize the final answer and one short check hint, while long worked steps remain in the saved data for reports and audits instead of filling the parent checking view.
- v23.241 further declutters answer-key screens: daily mix, submission progress, post-submit actions, and Closure write-back guidance now render as compact status strips so the answer rows stay visually dominant during parent checking.
- v23.242 adds an answer quick-scan mode: parents can switch an on-screen answer sheet to answer-only rows while answer-sheet printing defaults to the same compact number-plus-answer layout, reducing search time when checking against paper.
- v23.243 reorganizes the dedicated error book around quick printable practice packs: parents can choose recommended 10-question, extended 20-question, wrong-again priority, or full-scope sheets; grading records preserve the chosen size and surface a next-day plan so spaced retrieval stays clear and practical.
